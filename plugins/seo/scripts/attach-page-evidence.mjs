...
import { insideDataRoot, safeId } from "../src/data-root.mjs";
...
const detail = await store.getPage(auditId, pageId), screenshots = [];
const folder = insideDataRoot("audits", auditId, "pages", pageId, "assets");
...
async function saveImage(source, basename, label) {
  ...
  const target = insideDataRoot("audits", auditId, "pages", pageId, "assets", name), temporary = `${target}.${randomUUID()}.tmp`;
  await writeFile(temporary, bytes, { mode: 0o600 }); await chmod(temporary, 0o600); await rename(temporary, target);
...
if (!page.performance || Object.keys(page.performance).length === 0) page.diagnostics.push({ code: "deep-performance-data-pending", message: "Las capturas y el DOM están disponibles, pero faltan métricas de rendimiento verificadas.", completenessImpact: "La evidencia visual está completa; CWV/Lighthouse permanece sin cobertura.", nextAction: "Ejecutar Lighthouse o consultar CrUX y guardar únicamente los resultados obtenidos." });
