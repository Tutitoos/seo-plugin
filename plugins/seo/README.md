...
Plugin Codex con 54 skills y 36 herramientas MCP. Incluye la orquestadora `/seo full`, 50 Marketing Skills y las integraciones Analytics, Search Console y Business Profile con perfiles conjuntos y credenciales aisladas por servicio y cuenta.
...
La pestaña **Perfil de empresa** combina la ficha, galería, servicios, atributos, reseñas, publicaciones, diagnósticos y rendimiento local. El contenido obtenido de Google se guarda en una caché privada de hasta 30 días, fuera del snapshot y de su cuota de 512 MB; después se oculta hasta ejecutar una auditoría nueva.
...
La API local añade `get_audit_run_status`, `get_audit_changes`, `get_audit_storage`, `save_business_profile_capture` y `get_business_profile_capture`. Los snapshots v1–v4 se siguen leyendo; los nuevos se guardan como v5 con referencia temporal al Perfil de empresa, `changes.json`, cobertura por página, idiomas esperados/declarados y diferencias frente al snapshot anterior.
