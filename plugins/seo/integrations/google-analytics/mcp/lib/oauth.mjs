...
export const GOOGLE_SCOPES = Object.freeze(["openid", "email", ANALYTICS_SCOPE]);
...
function assertGoogleEndpoint(raw, expectedHost, field) {
  let parsed;
  try { parsed = new URL(raw); } catch { throw new Error(`El campo ${field} del JSON OAuth no contiene una URL válida.`); }
...
export async function readDesktopCredentials(credentialsFile) {
  if (!isAbsolute(credentialsFile)) throw new Error("credentialsFile debe ser una ruta absoluta al JSON OAuth descargado.");
  const info = await stat(credentialsFile);
  if (!info.isFile() || info.size > 64 * 1024) throw new Error("El archivo de credenciales no es un JSON OAuth de tamaño válido.");
  ...
  if (!installed.client_id.endsWith(".apps.googleusercontent.com")) throw new Error("El client_id del JSON OAuth no tiene el formato esperado de Google.");
  return {
    clientId: installed.client_id,
    clientSecret: installed.client_secret,
    authUri: assertGoogleEndpoint(installed.auth_uri || "https://accounts.google.com/o/oauth2/v2/auth", "accounts.google.com", "auth_uri"),
    tokenUri: assertGoogleEndpoint(installed.token_uri || "https://oauth2.googleapis.com/token", "oauth2.googleapis.com", "token_uri"),
  };
}
...
function accessExpiry(expiresIn) {
  ...
  return Date.now() + (Number.isFinite(lifetime) ? Math.max(0, lifetime - 60) : 3540) * 1000;
}

function html(message, success) {
  const color = success ? "#137333" : "#b3261e";
  return `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'"><title>Google Analytics</title><style>body{font:16px -apple-system,BlinkMacSystemFont,sans-serif;max-width:680px;margin:15vh auto;padding:24px;color:#202124}h1{color:${color}}</style></head><body><h1>${success ? "Autorización recibida" : "No se pudo autorizar"}</h1><p>${message}</p><p>Ya puedes cerrar esta pestaña y volver a Codex.</p></body></html>`;
}
...
export async function waitForAuthorizationCode({ authUrl, state, openBrowser = defaultOpenBrowser, timeoutMs = 240_000 }) {
  ...
  const timeout = setTimeout(() => {
    if (!settled) { settled = true; rejectCode(new Error("La autorización OAuth agotó el tiempo de espera.")); }
  }, timeoutMs);
  try {
...
export class GoogleOAuthManager {
  ...
  async status({ verify = true } = {}) {
    ...
    return { connected: true, needsReconnect: false, accountEmail: token.email, scopes: token.scopes || GOOGLE_SCOPES };
  }
  ...
  async connect({ credentialsFile, forceAccountSelection = false, preferredAccountEmail } = {}) {
    ...
    if (existing?.refreshToken && !forceAccountSelection) {
      return { ...(await this.status()), message: "Ya hay una cuenta conectada. Usa forceAccountSelection para sustituirla." };
    }
    ...
    const expectedEmail = normalizeEmail(preferredAccountEmail);
    ...
    const { code, redirectUri } = await waitForAuthorizationCode({ authUrl: authUrl.toString(), state, openBrowser: this.openBrowser, timeoutMs: this.timeoutMs });
    const response = await this.fetchImpl(config.tokenUri, {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams({ client_id: config.clientId, client_secret: config.clientSecret, code, code_verifier: verifier, grant_type: "authorization_code", redirect_uri: redirectUri }),
    });
    const tokens = await parseJsonResponse(response, "No se pudo intercambiar el código OAuth");
    if (!tokens.refresh_token || !tokens.access_token) throw new Error("Google no devolvió un refresh token. Revoca el acceso anterior y vuelve a conectar.");
    const userInfoResponse = await this.fetchImpl("https://openidconnect.googleapis.com/v1/userinfo", { headers: { Authorization: `Bearer ${tokens.access_token}` } });
    const userInfo = await parseJsonResponse(userInfoResponse, "No se pudo identificar la cuenta conectada");
    const actualEmail = normalizeEmail(userInfo.email);
    const scopes = typeof tokens.scope === "string" ? tokens.scope.split(/\s+/).filter(Boolean) : GOOGLE_SCOPES;
    if (!scopes.includes(ANALYTICS_SCOPE)) throw new Error("Google no concedió el permiso de solo lectura de Analytics.");
    ...
    await this.keychain.set(TOKEN_ACCOUNT, { refreshToken: tokens.refresh_token, email: actualEmail || "Cuenta de Google", scopes });
    this.access = { token: tokens.access_token, expiresAt: accessExpiry(tokens.expires_in) };
    return { connected: true, needsReconnect: false, accountEmail: actualEmail || "Cuenta de Google", scopes };
  }

  async getAccessToken({ forceRefresh = false } = {}) {
    if (!forceRefresh && this.access?.token && this.access.expiresAt > Date.now()) return this.access.token;
    const [config, saved] = await Promise.all([this.keychain.get(CLIENT_ACCOUNT), this.keychain.get(TOKEN_ACCOUNT)]);
    if (!config?.clientId || !saved?.refreshToken) throw new Error("No hay una cuenta conectada. Usa manage_analytics_connection con action=connect.");
    const response = await this.fetchImpl(config.tokenUri, {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams({ client_id: config.clientId, client_secret: config.clientSecret, refresh_token: saved.refreshToken, grant_type: "refresh_token" }),
    });
    try {
  ...
  async disconnect() {
    ...
    if (!saved?.refreshToken) return { connected: false, revoked: false, message: "No había ninguna cuenta conectada." };
    let response;
    let lastNetworkError;
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        response = await this.fetchImpl("https://oauth2.googleapis.com/revoke", {
          method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ token: saved.refreshToken }),
        });
        if ((response.status === 429 || response.status >= 500) && attempt < 2) { await this.sleep(retryDelay(response, attempt)); continue; }
        break;
      } catch (error) {
    ...
    if (!response) throw new Error(`No se pudo contactar con Google para revocar la sesión; se conserva el token para reintentar: ${lastNetworkError?.message || "error de red"}`);
    if (!response.ok && response.status !== 400) throw new Error(`Google no confirmó la revocación (HTTP ${response.status}); se conserva el token para reintentar.`);
    await this.keychain.delete(TOKEN_ACCOUNT);
    this.access = undefined;
