...
function html(message, success) {
  ...
  return `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'"><title>Google Business Profile</title><style>body{font:16px -apple-system,BlinkMacSystemFont,sans-serif;max-width:680px;margin:15vh auto;padding:24px;color:#202124}h1{color:${color}}</style></head><body><h1>${success ? "Autorización recibida" : "No se pudo autorizar"}</h1><p>${message}</p><p>Ya puedes cerrar esta pestaña y volver a Codex.</p></body></html>`;
}
...
export class GoogleOAuthManager {
  ...
  async connect({ credentialsFile, forceAccountSelection = false, preferredAccountEmail } = {}) {
    ...
    if (!scopes.includes("https://www.googleapis.com/auth/business.manage")) {
      throw new Error("Google no concedió el permiso de Google Business Profile.");
    }
  ...
  async getAccessToken({ forceRefresh = false } = {}) {
    ...
    if (!config?.clientId || !saved?.refreshToken) {
      throw new Error("No hay una cuenta conectada. Usa manage_business_profile_connection con action=connect.");
    }
    const response = await this.fetchImpl(config.tokenUri, {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
